    ...
    <section className="container relative m-auto mt-12 grid grid-cols-1 grid-rows-1 gap-4 px-14 py-10 md:grid-cols-2 lg:py-16">
      <div>
        ...
        <p className="mt-2 text-center font-fontRoboto text-palletteColor6 md:text-left">
          Our customers are delighted with the exceptional service and high-quality products they receive from us. Here are some of their
          testimonials:
        </p>
        <div className="flex flex-col items-center py-8 font-fontRoboto text-palletteColor5 md:flex-row md:justify-between md:pr-24 lg:pr-64">
          ...
          <div className="items:center flex flex-col text-center md:text-left">
            ...
            <div className="flex flex-row justify-center md:justify-start">
              <svg xmlns="http://www.w3.org/2000/svg" className="size-4" style={{ fill: '#FFC107' }} viewBox="0 0 1000 1000">
      ...
      <div>
        <Carousel swipeable={false} removeArrowOnDeviceType={['tablet', 'mobile', 'desktop']} autoPlay responsive={responsive}>
          {Persons.map((person, index) => (
            <div className="mt-16 flex flex-col items-center md:mt-4 md:items-start" key={index}>
              <img src={person.img} className="h-20 w-20 content-center rounded-full" alt="Person image" />
              <h2 className="mt-4 font-fontOpen text-[1.125rem] font-semibold text-[#0d0925]">{person.name}</h2>
              <p className="font-fontOpen text-heading6 text-palletteColor13">{person.role}</p>
              <p className="mt-6 text-center font-fontOpen text-heading6 font-medium text-[#31343A] md:text-left">{person.comment}</p>
            </div>
          ))}
        </Carousel>
      </div>
