          ...
          {({ isActive }) => (
            <>
              ...
              <img
                ...
                style={{
                  filter: isActive ? 'invert(40%) sepia(52%) saturate(4000%) hue-rotate(190deg) brightness(100%) contrast(100%)' : 'none',
                }}
                src={chevron}
                alt="Chevron"
              />
...
const AccordionsubItem = ({ header, isActive, ...rest }) => (
  <Item
    ...
    header={({ state: { isEnter } }) => (
      <>
        <motion.svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="mr-4">
          <motion.line
            x1="4"
            y1="12"
            x2="20"
        ...
        <div className="font-fontOpen text-base" style={{ textTransform: 'uppercase' }}>
          {header}
        </div>
        <img
    ...
    <>
      ...
      <motion.aside
        ...
        <div className="z-40 flex p-4 text-black">
          <span className="font-fontOpen text-heading6 font-semibold text-palletteColor5">Menu</span>
        ...
        <div className="mx-2 my-4 text-black">
          ...
          <ControlledAccordion providerValue={providerValue}>
            {MenuItems.map((item, index) =>
              item.items ? (
                <AccordionItem key={index} header={item.title} link={item.link} className=" ">
                  <Accordion>
                    {item.items.map((subitem, index) => (
                      <AccordionsubItem key={index} header={subitem.title} link={subitem.link} className="">
                        {subitem.subItems
                                ...
                                {({ isActive }) => (
                                  <div
                                    ...
                                    <svg
                                      ...
                                      className="mr-4">
                                      <motion.circle cx="4" cy="12" r="2" fill="#D1D5DB" />
                                    </svg>
                                    {subsubitem.icon && (
                                      <img
                                        src={subsubitem.icon}
                                    ...
                                    <div className="w-4/5">
                                      <h5 className="mb-1.5 font-fontOpen text-14">{subsubitem.title}</h5>
                                    </div>
                                  </div>
                                )}
                              </NavLink>
        ...
        <NavLink to="/pricing" onClick={() => setToggleMobile(false)} className="mt-6 flex justify-center">
