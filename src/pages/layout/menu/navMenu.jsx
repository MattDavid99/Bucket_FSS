          ...
          <div className="hidden w-full lg:flex lg:pl-11" id="megamenu-cta">
            <ul className="my-4 flex flex-col gap-y-4 lg:my-0 lg:flex-row lg:items-center">
              {MenuItems.map((menuItem, index) => (
                <li key={index} className={`cursor-pointer ${menuItem.items ? '' : 'relative'}`}>
                  {!menuItem.items ? (
                    <NavLink
                      to={menuItem.link}
                      className={() =>
                        `flex items-center justify-between gap-x-2 text-14 font-semibold capitalize ${
                          isSticky ? 'text-white hover:text-palletteColor1' : 'text-palletteColor5 hover:text-palletteColor1'
                    ...
                    <NavLink
                      ...
                      {isHovered[menuItem.title] && menuItem.items && (
                        <AnimatePresence>
                          <motion.div
                            initial={{
                              opacity: 0,
                            ...
                            <div className="grid grid-cols-1 justify-between lg:grid-cols-3">
                              {menuItem.items.map((subItem, subIndex) => (
                                <ul key={subIndex} className="text-sm text-gray-700" aria-labelledby="dropdownLargeButton">
                                  <h6 className="mb-2 text-sm font-medium text-gray-500">{subItem.title}</h6>
                                  {subItem.subItems.map((item, itemIndex) => (
                                    <li key={itemIndex}>
                                      <NavLink
                                        to={item.link}
                                        className="group flex items-center py-2 transition-all duration-500 hover:rounded-xl hover:text-palletteColor1">
                                        {item.icon && <img src={item.icon} className="group-hover:filter-blue mr-2" alt={item.title} />}
                                        <div className="w-4/5">
